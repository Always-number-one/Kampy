          ...
          body: Stack(
            children: [
                                  ...
                                  builder: (context,AsyncSnapshot<QuerySnapshot> snapshot){
                                    ...
                                    if(snapshot.hasData){
                                      for (var element in snapshot.data!.docs) {
                                        var response =  _firestore.collection('Events').doc(element.id).collection('Likes').get().then((value){
                                          setState(() {
                                            likes++;
                                          });
                                        });
                                        cards.add(PostsCard(title: element['title'], uploader: element['uploader'], uploaderImage: element['uploaderImage'], photo: element['photo'], eventid: element.id,likes: likes.toString(),));
              ...
              Center(
                child: Visibility(
                  visible: visibility,
                  child: const CircularProgressIndicator(color: Color.fromARGB(255, 62, 10, 122),),
                ),
              )
            ],
          )),
...
class PostsCard extends StatefulWidget {
  ...
  final comments;
   ...
   PostsCard({Key? key,required this.title,required this.uploader,required this.uploaderImage , required this.photo, this.eventid, this.likes, this.comments,}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                StreamBuilder(
                  stream: _firestore.collection('Events').doc(widget.eventid).collection('Likes').snapshots(),
                  builder: (context,AsyncSnapshot<QuerySnapshot> snapshot) {
                    return MaterialButton(
                      ...
                      onPressed: ()async {
                        ...
                        if(like == false ){
                         ...
                         var response = await _firestore.collection('Events').doc(widget.eventid).collection('Likes').doc(name).set( {
                                'name': name});
                          ...
                          var response = await _firestore.collection('Events').doc(widget.eventid).collection('Likes').doc(name).delete();
                        }
                ...
                StreamBuilder(
                  ...
                  builder: (context,AsyncSnapshot<QuerySnapshot> snapshot) {
                    return MaterialButton(
                      ...
                      onPressed: () {
                        Navigator.push(context, MaterialPageRoute(builder: (context) =>Comments(eventid: widget.eventid,)));
                      },
                    );
                  }
                ),
